...
from kedro.io.core import Version, parse_dataset_definition
...
class BaseDataset:
    ...
    def __init__(
        ...
        load_args: dict,
        credentials: dict,
        save_args: dict = None,  # type: ignore
        path: str = None,
    ...
    def get_version(self):
        ...
        if (self.version_config is None) and (self.load_args.get("starting_weekday")):
            self.version_config["starting_weekday"] = self.load_args.pop(
    ...
    def generate_and_check_dtypes_dict(self):
        ...
        self.dtypes = (
            self.load_args.pop("dtypes") if "dtypes" in self.load_args.keys() else {}
        )
    ...
    def _generate_first_day(self):
        ...
        if self.version_config.get("starting_weekday"):
            days_difference = (
                today.weekday() - self.version_config["starting_weekday"]
            ) % 7
        else:
            days_difference = 0
...
class ReadParquet(ParquetDataSet, BaseDataset):  # VendasDataSet
    ...
    def __init__(
        ...
        super().__init__(
            self.path, self.load_args, self.save_args, version, self.credentials
        )
...
class VersionedDataset(AbstractVersionedDataSet, BaseDataset):  # VendasVersionedDataset
    ...
    def get_existing_versions(self):
        ...
        versions = [
            i for i in glob_result if i.startswith(name) and i.endswith(file_extension)
        ]
        versions.sort()
        return versions
    ...
    def _format_filepath_date(self) -> str:
        ...
        if date_formats:
            ...
            formatted_dates = {
                key: first_day.strftime(fmt) for key, fmt in date_formats.items()
            }
            formatted_filepath = formatted_filepath.format(**formatted_dates)
        if "{" in formatted_filepath and "}" in formatted_filepath:
            raise ValueError(
                f"""Unsupported format string in filepath: {formatted_filepath}, \n
    ...
    def _save(self, data: Union[pd.DataFrame, dict]) -> None:
        ...
        if isinstance(data, pd.DataFrame):
            ...
            raise ValueError(
                f"File extension not supported: {formatted_filepath_error_message}"
            )
...
class FileReader(AbstractDataSet, BaseDataset):
    ...
    def _get_date_string(self, path: str) -> pd.Timestamp:
        ...
        date_source = (
            self._get_date_source()
        )  # self.load_args.get('date_source', 'path')
    ...
    def _generate_last_day(self, first_day: pd.Timestamp) -> pd.Timestamp:
        ...
        last_day = first_day - pd.DateOffset(
            **{self.load_args["time_scale"]: self.load_args["history_length"]}
        )
    ...
    def _load(self) -> pd.DataFrame:
        ...
        with ThreadPoolExecutor(
            max_workers=self.load_args.get("thread_count", 12)
        ) as executor:
            dfs = pd.concat(
                executor.map(self._to_pandas_dataframe, path_list), ignore_index=True
            )
        logging.info(f"Loaded {self.path}")
        return dfs
