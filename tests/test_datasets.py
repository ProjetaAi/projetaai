from kedro_projetaai.utils.datasets.datasets import DatasetTypes, LoadLast, ReadFile, PathReader, VersionedDataset
...
def save_files(df: Union[pd.DataFrame, DataFrameGroupBy],
               format: str,
               path: str,
    ...
    if isinstance(df, DataFrameGroupBy):
        ...
        for name, group in df:
            ...
            datasetype_obj.write(group, path + '_' + name + suffix_name + '.' + format, storage_options={})
        return
...
def generate_dataframe(n_rows: int, n_cols: int = 2) -> pd.DataFrame:
    ...
    dates = pd.date_range(start=(pd.to_datetime('today') - pd.DateOffset(days=n_rows-1)).strftime('%Y-%m-%d'),
...
class test_datasets(unittest.TestCase):
    ...
    def test_ReadFile_excel(self):
        for extension in ['csv', 'xlsx']:
            ...
            readfile_obj =  ReadFile(path=TEMP_PREFIX + f'test.{extension}',
                                    credentials=None)
    ...
    def test_ReadFile_pickle(self):
        ...
        readfile_obj = ReadFile(path=TEMP_PREFIX + f'test.pickle',
                                    credentials=None)
        df_read = readfile_obj._load()
        self.assertIsInstance(df_read, pd.DataFrame)
        self.assertTrue(df.equals(df_read))
    ...
    def test_PathReader(self):
        ...
        save_files(df.groupby(df['date'].dt.strftime("%Y-%m-%d")), 'parquet', TEMP_PREFIX + 'test.parquet')
        readfile_obj = PathReader(path=TEMP_PREFIX,
                                  credentials=None,
                                  read_args={'time_scale': 'M',
                                             'history_length': months_test},
    ...
    def test_PathReader_t(self):
        ...
        save_files(df.groupby(df['date'].dt.strftime("%Y-%m-%d")), 'parquet', TEMP_PREFIX + 'test.parquet', name_group_edit=True)
        readfile_obj = PathReader(path=TEMP_PREFIX,
                                  credentials=None,
                                  read_args={'time_scale': 'M',
                                             'history_length': months_test},
    ...
    def test_PathReader_back_date(self):
        ...
        readfile_obj_no_back_date = PathReader(path=TEMP_PREFIX,
                                               credentials=None,
                                               read_args={'time_scale': 'M',
                                                          'history_length': months_test},
                                               load_args={'columns': ['date', 'col_0']}
    ...
    def test_VersionedDataSet(self):
        ...
        readfile_obj = VersionedDataset(path=filepath,
                                        credentials=None,
        ...
        self.assertFalse(os.path.exists(TEMP_PREFIX + f'/{today}/' + f'test_{today}.parquet'))
        readfile_obj._save(df_load)
        self.assertTrue(os.path.exists(TEMP_PREFIX + f'/{today}/' + f'test_{today}.parquet'))
        remove_files()
        return

    def test_abfs_path(self):
        path = 'abfs://account_name.dfs.core.windows.net/test/test.parquet'
        readfile_obj = ReadFile(path=path,
    ...
    def test_rises_VersionedDataset(self):
        with self.assertRaises(ValueError):
            readfile_obj = VersionedDataset(path='test{not_date_path}{date_path}/test{date_file}.parquet',
                                            credentials=None,
                                            version_config={'date_path': '%Y-%m-%d',
                                                            'date_file': '%Y-%m-%d'})
    ...
    def test_LoadLast(self):
        ...
        save_files(df.groupby(df['date'].dt.strftime("%Y-%m-%d")), 'parquet', TEMP_PREFIX + 'test.parquet', name_group_edit=True)
        readfile_obj = LoadLast(path=TEMP_PREFIX,
                                  credentials=None,
                                  load_args={'columns': ['date', 'col_0']})
        df_read = readfile_obj._load()
        self.assertTrue(max_date == df_read['date'].max())
    ...
    def test_LoadLast_backdate(self):
        df = generate_dataframe(300, 2)
        back_date = pd.to_datetime(df['date']).dt.strftime("%Y-%m-%d").sample(1).values[0]
        save_files(df.groupby(df['date'].dt.strftime("%Y-%m-%d")), 'parquet', TEMP_PREFIX + 'test.parquet', name_group_edit=True)
        readfile_obj = LoadLast(path=TEMP_PREFIX,
                                  credentials=None,
                                  load_args={'columns': ['date', 'col_0']},
    ...
    def test_dtypes(self):
        ...
        readfile_obj = ReadFile(path=TEMP_PREFIX + f'test.parquet',
                        credentials={'account_name': 'account_name',
                                     'anon': False},
